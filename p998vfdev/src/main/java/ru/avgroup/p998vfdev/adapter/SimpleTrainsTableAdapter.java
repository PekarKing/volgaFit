...
public class SimpleTrainsTableAdapter extends BaseAdapter implements Filterable, ThemedSpinnerAdapter {
    ...
    public static long[] ids = new long[100];
    ...
    public View getView(final int position, View convertView, ViewGroup parent) {
        ...
        if (convertView == null){
            ...
            viewHolder.chb = (CheckBox) convertView.findViewById(R.id.itemChb);
            convertView.setTag(viewHolder);
        } else {
            viewHolder = (ViewHolder) convertView.getTag();
        }

        viewHolder.chb.setOnCheckedChangeListener(null);
        viewHolder.chb.setFocusable(false);

        viewHolder.chb.setOnCheckedChangeListener(new CompoundButton.OnCheckedChangeListener() {
            @Override
            public void onCheckedChanged(CompoundButton buttonView, boolean isChecked) {
                if(isChecked){
                    m = new HashMap<String, Object>(mData.get(position));
                    ContentValues values = new ContentValues();
                    values.put(TrainsContract.TrainsEntry.COLUMN_TITLE, String.valueOf(m.get("title")));
                    values.put(TrainsContract.TrainsEntry.COLUMN_DESCR, String.valueOf(m.get("description")));
                    values.put(TrainsContract.TrainsEntry.COLUMN_TIME, String.valueOf(m.get("time")));
                    values.put(TrainsContract.TrainsEntry.COLUMN_GYM, String.valueOf(m.get("gym")));
                    values.put(TrainsContract.TrainsEntry.COLUMN_WEEKDAY, String.valueOf(m.get("weekday")));
                    long newRowId = db.insert(TrainsContract.TrainsEntry.TABLE_NAME, null, values);

                    ids[position] = newRowId;
                    Toast.makeText(context, "Добавлено в Мои занятия", Toast.LENGTH_SHORT).show();
                } else {
                    db.delete("trains", "_id = " + String.valueOf(ids[position]), null);
